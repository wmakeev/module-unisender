...
class UnisenderModule {
  ...
  callMethod(methodName, options) {
    return this.api[methodName](options).then(response => response.result).catch(response => {
      let error = maybe(response).filter(res => res.code && res.error).map(res => {
        if (errors[res.code]) {
